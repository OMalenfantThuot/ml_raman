...
def gamma_phonons_graphene(natoms_sc):
    nmodes = 3 * natoms_sc
    datapath = os.path.join(
        *__file__.split("/")[:-1], "modes", "gamma_modes_graphene.h5"
    )
    datapath = "/" + datapath
    print(datapath)
...
def gamma_eigendisplacements_graphene(natoms_sc):
    nmodes = 3 * natoms_sc
    datapath = os.path.join(
        *__file__.split("/")[:-1], "modes", "gamma_modes_graphene.h5"
    )
    ...
    with h5py.File(datapath, "r") as file:
        v1_pr = np.array(file["v1"])
        v2_pr = np.array(file["v2"])
        pristine_masses_pc = np.repeat(np.array([12.011, 12.011]), 3)
