...
def create_parser():
    ...
    parser.add_argument(
        ...
        help="concentration of added nitrogen.",
        type = str
    )
    parser.add_argument(
        "--index_range",
...
def main(args):
    ...
    nitrogen_conc = args.nitrogen_concentration 
    npts = args.npts
    width = args.width
    ...
    if args.index_range == None:
        ...
        f1 = h5py.File('/home/dounia/projects/rrg-cotemich-ac/dounia/datasets/raman/gra_nitrogen/phonons'+str(Mx)+'x'+str(My)+'_'+nitrogen_conc+'_phonons'+'.h5','r')
        ...
        for j in range(idx_min,idx_max+1):
            f1=h5py.File('/home/dounia/projects/rrg-cotemich-ac/dounia/datasets/raman/gra_nitrogen/phonons/'+str(Mx)+'x'+str(My)+'_'+nitrogen_conc+'_'+str(j)+'_phonons'+'.h5','r')
            key_energies = list(f1.keys())[0]
            key_modes = list(f1.keys())[1]
            # Get the data
    ...
    rdosax.set_title("Raman spectrum for "+str(Mx)+"x"+str(My)+" graphene sc at nitrogen conc "+str(nitrogen_conc), fontsize=8)
   ...
   # rdosax.plot(eigenval[int(peaks[0])],raman[int(peaks[0])],marker='.',label='peak at '+str('%.3f'%(eigenval[int(peaks[0])]))+' $\mathregular{cm^{-1}}$', markersize=10)
    ...
    if savefile:
        if width ==0:
            rdosax.plot(eig_avg[0], eig_avg[1], label="raman")
            np.savetxt('raman'+str(Mx)+"x"+str(My)+'_'+str(nitrogen_conc)+'_w'+str(width)+'.dat', np.transpose(eig_avg))
        else:
            rdosplot = rdos.sample_grid(npts=npts, width=width, smearing = smearing)
            raman_proj[1] = rdosplot.get_weights()
            raman_proj[0] = rdosplot.get_energies()
            np.savetxt('raman'+str(Mx)+"x"+str(My)+'_'+str(nitrogen_conc)+'_w'+str(width)+'.dat', np.transpose(raman_proj))
